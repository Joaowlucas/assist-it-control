...
export function useChat() {
  ...
  const { 
    ...
    queryFn: async () => {
      ...
      const roomsWithCounts = (data as ChatRoom[])?.map(room => ({
        ...room,
        participant_count: room.participants?.length || 0
      })) || []

      console.log('Chat rooms fetched:', roomsWithCounts.length, 'rooms')
  ...
  const { 
    ...
    queryFn: async () => {
      ...
      const messagesWithSender = (data as ChatMessage[])?.map(message => ({
        ...message,
        sender: message.profiles
      })) || []

      console.log('Messages fetched:', messagesWithSender.length, 'messages')
      return messagesWithSender
    },
    enabled: !!selectedRoom?.id && !!profile?.id,
  })
  ...
  useEffect(() => {
    if (!selectedRoom?.id || !profile?.id) return
  ...
  }, [selectedRoom?.id, profile?.id, queryClient])
  ...
  const sendMessage = async (roomId: string, content: string, attachmentFile?: File) => {
  ...
  const createRoom = async (params: {
    ...
    return createRoomMutation.mutateAsync(params)
  }
  ...
  const updateRoom = async (params: {
    ...
    return updateRoomMutation.mutateAsync(params)
  }
